...
resource "aws_subnet" "vpn_subnets" {
    for_each = tomap(var.environment.vpc.subnets.VPN)
    vpc_id = aws_vpc.vpc.id
    cidr_block = each.value
    availability_zone_id = join("-", [var.common_vars.region_id, each.key])

    tags = {
      "Name" = format("%s_%s_SBNT_%s_%s_VPN", var.common_vars.organization, var.environment.name, local.purpose, upper(each.key))
      environment = var.environment.name
    }
}
...
resource "aws_nat_gateway" "nat_gateway" {
    for_each = var.environment.vpc.subnets.VPN
    connectivity_type = "private"
    subnet_id = aws_subnet.vpn_subnets[each.key].id
    
    tags = {
      "Name" = format("%s_%s_NAT_VPN_%s", var.common_vars.organization, var.environment.name, upper(each.key))
      environment = var.environment.name
    }
}
...
resource "aws_route_table" "vpn_route_table" {
    ...
    tags = {
      "Name" = format("%s_%s_RTBL_%s_VPN", var.common_vars.organization, var.environment.name, local.purpose)
      environment = var.environment.name
    }
}
...
resource "aws_route_table_association" "vpn_route_table_associations" {
    for_each = var.environment.vpc.subnets.VPN
